...
namespace ORB_SLAM2
{
	...
	{
		cout << endl << "Saving camera trajectory to " << filename << "... " << std::flush;
		if (mSensor == MONOCULAR)
		{
	...
	{
		cout << endl << "Saving keyframe trajectory to " << filename << "... " << std::flush;
	...
	void System::SaveMap(const string &filename)
	{
		...
		cout << "Saving Map to " << filename << "... " << std::flush;
	...
	bool System::LoadMap(const string &filename)
	{
		...
		if (!in)
		{
			cerr << "Cannot Open Mapfile: " << filename << " , Create a new one" << endl;
			return false;
		}
		cout << "Loading Mapfile " << filename << "... "<< flush;
	...
	{
		cout << "Saving Mappoints to " << filename << "... " << std::flush;

		ofstream f;
		f.open(filename);
		f << fixed;

		vector<ORB_SLAM2::MapPoint*> points = mpMap->GetAllMapPoints();

		float x, y, z;
		for (int i = 0; i != points.size(); i++) {
			if (points[i]) {
				//if ( points[i]->isBad()) continue;
				cv::Mat coords = points[i]->GetWorldPos();
				f << coords.at<float>(0) << " " << coords.at<float>(1) << " " << coords.at<float>(2) << endl;
			}
		}
		f.close();
