...
void Viewer::Run()
{
	...
	pangolin::Var<bool> menuSave("menu.Save and Crash", false, false);
    ...
    while(1)
    {
		...
		if (menuSave)
		{
			mpSystem->SaveKeyFrameTrajectoryTUM("KeyFrameTrajectory.txt"); // timestamp tx ty tz qx qy qz qw
			mpSystem->SaveMapPoints("MapPoints.txt");
